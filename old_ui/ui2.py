...
class CodeAnonymizerApp:
    def __init__(self, root):
        ...
        # Anonymized Output
        self.anonymized_label = tk.Label(root, text="Anonymized Code and Prompt:", bg=self.bg_color, fg=self.fg_color)
        self.anonymized_label.grid(row=0, column=1, sticky="w", padx=10, pady=5)

        self.anonymized_text = scrolledtext.ScrolledText(root, wrap=tk.WORD, bg=self.entry_bg, fg=self.entry_fg, insertbackground=self.fg_color)
        self.anonymized_text.grid(row=1, column=1, rowspan=4, padx=10, pady=5, sticky="nsew")

        # Response Output
        self.response_label = tk.Label(root, text="API Response:", bg=self.bg_color, fg=self.fg_color)
        self.response_label.grid(row=0, column=2, sticky="w", padx=10, pady=5)

        self.response_text = scrolledtext.ScrolledText(root, wrap=tk.WORD, bg=self.entry_bg, fg=self.entry_fg, insertbackground=self.fg_color)
        self.response_text.grid(row=1, column=2, padx=10, pady=5, sticky="nsew")

        # Rebuild Output
        self.rebuild_label = tk.Label(root, text="Rebuilt Response:", bg=self.bg_color, fg=self.fg_color)
        self.rebuild_label.grid(row=2, column=2, sticky="w", padx=10, pady=5)

        self.rebuild_text = scrolledtext.ScrolledText(root, wrap=tk.WORD, bg=self.entry_bg, fg=self.entry_fg, insertbackground=self.fg_color)
        self.rebuild_text.grid(row=3, column=2, rowspan=2, padx=10, pady=5, sticky="nsew")
